...
- Sensitive credentials are encrypted at rest and are sent only to the service they belong to (GoHighLevel / OpenAI). If the user opts into a Clone2GHL account, funnels and account data may sync to the Clone2GHL backend
...
### 5. **alarms**
**Purpose:** Schedule a periodic background task to keep plan/usage state in sync.

**Justification:**
- Clone2GHL registers a single `chrome.alarms` alarm to periodically refresh the signed-in user's plan and usage state from the backend (and to drive lightweight background refresh such as watchlist checks)
- This is the MV3-recommended way to schedule periodic work from a service worker, instead of keeping a persistent timer alive
- The alarm performs no work unless the extension is installed; usage sync runs only when the user is signed in
- Implemented in `background.js` (the `USAGE_SYNC` alarm)
...
### **Clone2GHL backend (e.g., https://api.clone2ghl.com)**
- **Purpose:** Optional account sign-in, usage sync, server-side AI routing, and video workflows
- **Justification:** Network access to the backend is covered by the `<all_urls>` host permission; requests are made only after the user signs in and uses cloud features
...
2. **No Data Harvesting:** No tracking pixels, advertising, or data sales. Cloned HTML and settings are stored locally; server-side storage occurs only if the user creates a Clone2GHL account.
...
| alarms | Low | Single periodic sync alarm; accesses no page or personal data |
...
**Contact:** clone2ghl@gmail.com · https://clone2ghl.com
